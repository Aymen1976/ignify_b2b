...
# Nettoyage de texte pour éviter les erreurs dans le PDF
def clean_text(text):
    if not text:
        return ""
...
# Génération d’un PDF filtré
def generate_filtered_pdf(articles, filename="filtered_report.pdf"):
    pdf_file = Path("output") / filename
